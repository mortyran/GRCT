...
clusterExport(cl, c("ever_smoke_gss", "lung_cancer_gss", "genome_region", "ylab_gss_to_coloc_input"))
clusterEvalQ(cl, {
  library(data.table)
  library(dplyr)
  library(coloc)
...
coloc_h4_index = parLapply(cl, c(1:nrow(genome_region)), function(x){
  ...
  tryCatch({
    coloc_result <- coloc.abf(dataset1=ever_smoke_coloc_input,
                              dataset2=lung_cancer_coloc_input)
    if(as.numeric(coloc_result$summary[6]) > 0.3 & any(coloc_result$results$SNP.PP.H4 > 0.5)){
      return(x)
    }else{
      return(NA)
    }
